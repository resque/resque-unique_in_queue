...
- One spec file per class/module. For each class or module under `lib/`, keep all of its unit tests in a single spec file under `spec/` that mirrors the path and file name exactly: `lib/resque/unique_in_queue/my_class.rb` -> `spec/resque/unique_in_queue/my_class_spec.rb`.
...
[🧪build]: https://github.com/resque/resque-unique_in_queue/actions
